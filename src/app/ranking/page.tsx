  ...
  return (
    <main className="min-h-screen bg-slate-950 px-6 py-12 text-white">
      <div className="mx-auto max-w-5xl">
        <h1 className="mb-6 text-4xl font-bold">Ranking general</h1>

        {loading ? (
          <p className="text-slate-300">Cargando ranking...</p>
        ) : scores.length === 0 ? (
          <p className="text-slate-300">Todavía no hay puntuaciones guardadas.</p>
        ) : (
              ...
              <tbody>
                {scores.map((entry, index) => (
                  <tr key={entry.id} className="border-b border-slate-800 last:border-0">
                    ...
                    <td className="px-4 py-4">{entry.quizTitle}</td>
